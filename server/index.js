    ...
    const branchesWithEmployees = await Promise.all(
      branches.map(async (branch) => {
        ...
        const processedEmployees = await Promise.all(employees.map(async (emp) => {
          ...
          // Agar sotuvchi bo'lsa va oxirgi savdo sanasi bugun emas bo'lsa, 0 ga qaytaramiz
          if (emp.position === 'sotuvchi' && emp.lastSalesDate && emp.lastSalesDate !== today) {
            dailySales = 0;
            wholesaleSales = 0;
...
app.post('/api/employees', async (req, res) => {
  try {
    // dailyTasks ni shunchaki qabul qilamiz (agar berilgan bo'lsa)
