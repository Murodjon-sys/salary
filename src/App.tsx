...
export default function App() {
  ...
  const addEmployee = async () => {
    ...
    try {
      // Agar sotuvchi bo'lsa, barcha vazifalarni false qilib qo'yamiz (default)
      const employeeData = newEmployee.position === "sotuvchi" 
        ? {
            ...newEmployee,
            dailyTasks: {
              onTime: false,
  ...
  const calculateSalary = (employee: Employee) => {
    ...
    if (employee.position === "sotuvchi") {
      ...
      // Agar vazifalar yo'q bo'lsa, oddiy hisoblash
      const retailSales = employee.dailySales || 0;
      const wholesaleSales = employee.wholesaleSales || 0;
      const retailSalary = (retailSales * percentage) / 100;
      const wholesaleSalary = (wholesaleSales * percentage) / 100 / 2;
      return retailSalary + wholesaleSalary;
    }
        ...
        <div className="p-4 border-t border-gray-800 flex-shrink-0">
          {!isAuthenticated ? (
            <button
              ...
              className="w-full px-4 py-4 rounded-xl text-sm font-bold transition-all bg-gradient-to-r from-[#F87819] to-[#ff8c3a] text-white hover:shadow-2xl hover:shadow-orange-500/50 hover:scale-[1.02] shadow-lg"
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Ism</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Lavozim</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Foiz (%)</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Chakana Savdo</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Optom Savdo</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Oylik</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Amallar</th>
                    </tr>
                  </thead>
                    ...
                    {currentBranch.employees.map((employee) => (
                      <tr key={employee.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 bg-gray-900 rounded-lg flex items-center justify-center text-white text-sm font-bold shadow-md">
